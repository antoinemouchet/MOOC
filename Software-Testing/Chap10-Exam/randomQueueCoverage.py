...
def random_test():
 # Write a random tester for the Queue class
    ...
    for test in range(25):
        ...
        # Define a value to check if exception was raised or not
        testvalue = True

        # Random choose between enqueue and dequeue
        if (random.random() < 0.5):

            # Enqueue random number
            q.enqueue(randNum)
            # Check if an assertion was raised            
            try:
                q.checkRep()
            
            except AssertionError:
                successList.append((randNum, 1))
                testvalue = False
        
            # Make sure no exception occured
            if testvalue:
                successList.append((randNum, 0))       
            ...
            try:
                ...
                successList.append(('dq', 1))
                testvalue = False
            
            # Check no assertionError was found
            if testvalue:
                successList.append(('dq', 0))
